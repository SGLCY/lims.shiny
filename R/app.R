...
# Filtering ---------------------------------------------------------------
    ...
    data_columns <- reactive(names(data_to_use()))
   ...
   filtered_data <- reactive({
    ...
    data_merged <- reactive({
      ...
      data_to_use()[selected_rows(), ] %>% 
            ...
            mutate(
                LB_res = res_num,
                MB_res = if_else(res_num == 0 , detect_lim/2, res_num),
                UB_res = if_else(res_num == 0 , detect_lim, res_num),
            ) %>% 
            # MERGE with FoodEx1
            left_join(
    ...
    tbl_aggregate <- reactive({
      ...
      list(
        level2 = aggregate_occurrence(dt, .aggr_level = fdx1_group_level2),
        level3 = aggregate_occurrence(dt, .aggr_level = fdx1_group_level3)
      )
    ...
    output$tbl_aggregate <- DT::renderDataTable({
      ...
      table_to_view %>% 
        ...
        DT::formatRound(columns = columns, digits = digits ) %>% 
    ...
    output$down_excel <- downloadHandler(
      ...
      content = function(file){
        ...
        dt_level2 <- 
          ...
          mutate(across(-all_of(c(fdx1_group_level2, "N")), ~round(., digits = digits))
          ) %>% 
        ...
        dt_level3 <- 
          ...
          mutate(across(-all_of(c(fdx1_group_level3, "N")), ~round(., digits = digits))
          ) %>% 
          # remove empty rows
          filter(!is.na(N))
